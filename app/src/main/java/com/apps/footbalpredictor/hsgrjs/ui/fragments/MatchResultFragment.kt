...
import com.apps.footbalpredictor.hsgrjs.databinding.FragmentMatchResultBinding
...
class MatchResultFragment : Fragment() {
    ...
    private fun setupObservers() {
        viewModel.currentMatch.observe(viewLifecycleOwner) { match ->
            match?.let {
                binding.tvFinalScore.text = "${it.homeScore} - ${it.awayScore}"
                binding.tvHalfTimeScore.text = "(First Half: ${it.halfTimeHomeScore} - ${it.halfTimeAwayScore})"
            }
        }
    ...
    private fun setupClickListeners() {
        binding.btnMainMenu.setOnClickListener {
            ...
            findNavController().navigate(R.id.action_matchResultFragment_to_mainMenuFragment)
        }
    }
